...
impl InitiateVaultLockFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::initiate_vault_lock::builders::InitiateVaultLockInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.initiate_vault_lock().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::initiate_vault_lock::builders::InitiateVaultLockInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
