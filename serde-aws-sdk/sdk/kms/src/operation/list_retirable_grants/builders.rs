...
impl ListRetirableGrantsFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::list_retirable_grants::builders::ListRetirableGrantsInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.list_retirable_grants().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::list_retirable_grants::builders::ListRetirableGrantsInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
