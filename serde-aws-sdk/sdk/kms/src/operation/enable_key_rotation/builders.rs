...
impl EnableKeyRotationFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::enable_key_rotation::builders::EnableKeyRotationInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.enable_key_rotation().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::enable_key_rotation::builders::EnableKeyRotationInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
