...
impl BatchWriteItemFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::batch_write_item::builders::BatchWriteItemInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.batch_write_item().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::batch_write_item::builders::BatchWriteItemInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
