...
impl ListHostedZonesByVPCFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::list_hosted_zones_by_vpc::builders::ListHostedZonesByVpcInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.list_hosted_zones_by_vpc().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::list_hosted_zones_by_vpc::builders::ListHostedZonesByVpcInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
