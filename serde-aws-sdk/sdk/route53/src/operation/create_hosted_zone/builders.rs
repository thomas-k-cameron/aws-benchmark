...
impl CreateHostedZoneFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::create_hosted_zone::builders::CreateHostedZoneInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.create_hosted_zone().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::create_hosted_zone::builders::CreateHostedZoneInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
