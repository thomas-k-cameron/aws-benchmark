...
impl DisableHostedZoneDNSSECFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::disable_hosted_zone_dnssec::builders::DisableHostedZoneDnssecInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.disable_hosted_zone_dnssec().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::disable_hosted_zone_dnssec::builders::DisableHostedZoneDnssecInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
