...
impl GetHealthCheckLastFailureReasonFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::get_health_check_last_failure_reason::builders::GetHealthCheckLastFailureReasonInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.get_health_check_last_failure_reason().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::get_health_check_last_failure_reason::builders::GetHealthCheckLastFailureReasonInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
