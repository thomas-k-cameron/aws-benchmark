...
impl SubmitTaskStateChangeFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::submit_task_state_change::builders::SubmitTaskStateChangeInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.submit_task_state_change().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::submit_task_state_change::builders::SubmitTaskStateChangeInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
