...
impl DisableFastSnapshotRestoresFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::disable_fast_snapshot_restores::builders::DisableFastSnapshotRestoresInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.disable_fast_snapshot_restores().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::disable_fast_snapshot_restores::builders::DisableFastSnapshotRestoresInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
