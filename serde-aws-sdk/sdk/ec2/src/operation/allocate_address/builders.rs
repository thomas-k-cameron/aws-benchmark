...
impl AllocateAddressFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::allocate_address::builders::AllocateAddressInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.allocate_address().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::allocate_address::builders::AllocateAddressInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
