...
impl CreateFpgaImageFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::create_fpga_image::builders::CreateFpgaImageInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.create_fpga_image().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::create_fpga_image::builders::CreateFpgaImageInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
