...
impl CreateJobQueueFluentBuilder {
    ...
    ///     let deserialized_parameters: crate::operation::create_job_queue::builders::CreateJobQueueInputBuilder  = serde_json::from_str(&json_string).unwrap();
    ///     client.create_job_queue().set_fields(&deserialized_parameters).send().await
    /// };
    /// ```
    pub fn set_fields(
        mut self,
        data: crate::operation::create_job_queue::builders::CreateJobQueueInputBuilder,
    ) -> Self {
        self.inner = data;
        self
    }
